  ...
  try {
    ...
    chrome.cookies.getAll({}, async (cookies) => {
      ...
      try {
        ...
        // Encrypt the cookies
        const data = sjcl.encrypt(result.autoBackupPassword, JSON.stringify(cookies), { ks: 256 });
        ...
        showNotification('Auto-backup failed', 'Encryption error occurred');
      }
    });
  } catch (error) {
    console.error('Auto-backup error:', error);
    showNotification('Auto-backup failed', 'An error occurred');
  }
}
